...
export const DesignerV2_1: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (cached.sourceImageBase64 && cached.sourceImageMimeType) {
      ...
      console.log('[Cache Restore] ⚠️ No user image found in cache');
    }
    
    if (cached.fabricImageBase64 && cached.fabricImageMimeType) {
      console.log('[Cache Restore] 📥 Restoring fabric image:', {
        base64Length: cached.fabricImageBase64.length,
      ...
      try {
        ...
        console.log('[Cache Restore] ✅ Fabric preview URL created:', fabricUrl.substring(0, 50));
      } catch (error) {
        console.error('[Cache Restore] ❌ Failed to create fabric preview URL:', error);
      }
    } else {
      console.log('[Cache Restore] ⚠️ No fabric image found in cache');
    }
  ...
  React.useEffect(() => {
    ...
    persistTimerRef.current = window.setTimeout(() => {
      ...
      console.log('[Cache Persist] 💾 Saving state:', {
        hasUserImage: !!persistedSourceBase64,
        userImageLength: persistedSourceBase64?.length || 0,
        hasFabricImage: !!fabricImageBase64,
        fabricImageLength: fabricImageBase64?.length || 0,
        tabStates: {
  ...
  const handleFabricSwap = React.useCallback(async (opts?: { lightingPreset?: LightingPreset }) => {
    ...
    if (!user) {
      ...
      openUpgradeModal('no_user_account');
      return;
    }
        ...
        if (user?.uid) {
          try {
            ...
            if (currentUser) {
              ...
              console.log('🔐 [DesignerV2.1] Auth token obtained for user:', user.uid.substring(0, 8) + '...');
            }
          } catch (e) {
            console.warn('⚠️ [DesignerV2.1] Failed to get auth token:', e);
            // Continue without token - component will handle it
          }
        }
        
        const res = await generateFabricSwap({ ...payload, authToken });
      ...
      {isAdminUser && (
        <DebugPanel
          data={{
            ...
            TabState: {
              studio: studioImageBase64 ? 'Populated' : 'Empty',
              shop: shopImageBase64 ? 'Populated' : 'Empty',
              closet: closetImageBase64 ? 'Populated' : 'Empty',
            },
            TabStateData: {
              studio: studioImageBase64 ? `${studioImageBase64.substring(0, 30)}...` : 'null',
              shop: shopImageBase64 ? `${shopImageBase64.substring(0, 30)}...` : 'null',
