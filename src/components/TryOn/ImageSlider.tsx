...
const ImageSlider: React.FC<ImageSliderProps> = ({
  ...
  return (
    <div
      ...
      dir="ltr"
      className={
        `relative w-full ${heightClassName} overflow-visible select-none bg-gradient-to-r from-black via-black to-green-500 ` +
        (className ?? '')
      }
    ...
    >
      <div className="absolute inset-0 overflow-hidden rounded-lg bg-transparent">
        ...
        {!before && !beforeImageError && (
          <div className="absolute inset-0 flex items-center justify-center bg-orange-500">
            <div className="text-white text-xl font-bold tracking-widest">BEFORE</div>
          </div>
        )}
        ...
        {!after && !afterImageError && (
          ...
          >
            <div
              className="absolute top-0 left-0 bottom-0 flex items-center justify-center bg-blue-500"
              style={
                containerSize
                  ? { width: `${containerSize.width}px` }
                  : { width: '100%' }
            ...
            >
              <div className="text-white text-xl font-bold tracking-widest">AFTER</div>
            </div>
          </div>
        )}
      ...
      {/* Slider Handle Line */}
      <div
        className="absolute top-0 bottom-0 w-1 bg-white/90 flex items-center justify-center cursor-col-resize pointer-events-auto z-50"
        style={{ left: `${sliderPos}%`, transform: 'translateX(-50%)', willChange: 'left' }}
        onPointerDown={onStartDrag}
        ref={handleLineRef}
      >
        <div 
          className="w-10 h-10 bg-white border-2 border-theme-primary/10 rounded-full flex items-center justify-center cursor-col-resize pointer-events-auto touch-none z-50 transition-transform active:scale-90"
        >
