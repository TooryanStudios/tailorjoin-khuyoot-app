...
export const ReturnPolicyModal: React.FC<ReturnPolicyModalProps> = ({ isOpen, onClose, content }) => {
    ...
    >
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between bg-zinc-950 border-b border-zinc-800 p-4">
          <h2 className="text-lg font-bold text-white">سياسة الاسترجاع</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          <div className="rounded-xl border border-slate-200 bg-slate-50 text-slate-900">
            <div className="max-h-[45vh] overflow-y-auto p-4">
              <div className="whitespace-pre-wrap text-sm leading-relaxed">
                {content || 'لم يتم تحديد سياسة الاسترجاع حتى الآن.'}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 text-xs text-zinc-200">
            <input
              id="return-policy-agree"
              type="checkbox"
              className="mt-0.5 h-4 w-4"
              checked={agreed}
              onChange={(e) => setAgreed(e.target.checked)}
            />
            <label htmlFor="return-policy-agree">أقر أنني قرأت سياسة الاسترجاع وأوافق عليها.</label>
          </div>
