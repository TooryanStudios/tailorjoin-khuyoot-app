...
function TryOnResultComponent(props: TryOnResultProps, ref: React.Ref<HTMLDivElement>) {
  ...
  const drawerPortal = showDrawer && portalTarget && createPortal(
    <div
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
          <h3 className="text-xl font-bold text-slate-900 dark:text-white">معاينة كاملة</h3>
          <button
            onClick={() => setShowDrawer(false)}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {hasComparisonPanel && comparisonBeforeImage && comparisonAfterImage ? (
            <div className="max-w-2xl mx-auto">
              <ImageComparisonSlider
                beforeImage={comparisonBeforeImage}
                afterImage={comparisonAfterImage}
                beforeLabel={comparisonBeforeLabel}
                afterLabel={comparisonAfterLabel}
                aspectRatio="auto"
              />
            </div>
          ) : hasResultImage && effectiveResultImageSrc ? (
            <div className="max-w-2xl mx-auto">
              <img
                src={effectiveResultImageSrc}
                alt="نتيجة التصميم"
  ...
  return (
    <>
      <div ref={ref} className="relative">
        <div className="grid grid-cols-1 gap-3 items-start lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-4">
          {/* ========================================
              PANEL 1: COMPARISON (Mobile: below controls, Desktop: left)
              ======================================== */}
        <div className="lg:col-start-1 lg:row-start-1 relative z-0">
                        ...
                        <div className="p-2 space-y-1">
                          ...
                          {onToggleAdminAnchors ? (
                            <button
                              ...
                              className={
                                `w-full px-3 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex items-center gap-3 text-right ` +
        ...
        {/* ========================================
            PANEL 2: SIDEBAR (Desktop right) + CONTROLS + GENERATIONS
            ======================================== */}
        <div className="lg:col-start-2 lg:row-start-1 relative z-10">
          <div className="space-y-3 lg:sticky lg:top-3 lg:max-h-[calc(100vh-24px)] lg:flex lg:flex-col">
            {/* Controls card */}
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-1 space-y-1">
        ...
        {/* ========================================
            PANEL 3: COMPARISON (Mobile: below controls, Desktop: right)
            ======================================== */}
        <div className="lg:col-start-3 lg:row-start-1 relative z-0">
          <div ref={comparisonPanelRef} className="w-full max-w-[520px] mx-auto space-y-3">
            <div className="tryon-comparison-panel rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3">
              <div className="flex items-center justify-between gap-3">
                <div>
        ...
        <div className="lg:col-start-2 lg:row-start-1 relative z-10">
          <div className="space-y-3 lg:sticky lg:top-3 lg:max-h-[calc(100vh-24px)] lg:flex lg:flex-col">
            ...
            {/* Generations rail area (the red-marked area) */}
            <div className="hidden lg:flex lg:flex-col rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900/70 backdrop-blur p-2 min-h-[180px] flex-1">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm font-black text-slate-900 dark:text-white">إبداعاتي ({modalGenerations?.length ?? 0})</div>
              </div>

              <div className="flex-1 overflow-y-auto pr-1 space-y-2" style={{ scrollbarGutter: 'stable both-edges' }}>
                {(modalGenerations ?? []).slice(0, 15).map((g) => {
                  ...
                  return (
                    <div
                      ...
                      className="group relative w-full aspect-[3/4] rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 hover:opacity-95 cursor-pointer"
                      title="عرض الصورة"
                    >
                      <img
                ...
                {Array.from({ length: placeholderCount }).map((_, idx) => (
                  <div
                    ...
                    className="w-full aspect-[3/4] rounded-xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40"
                    aria-hidden="true"
                  />
                ))}
              </div>
      ...
      {/* ========================================
       ...
       ========================================= */}
      {showDrawer && portalTarget && createPortal(
        <div
          className="fixed inset-0 z-[999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => setShowDrawer(false)}
        >
