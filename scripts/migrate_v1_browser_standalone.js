/**
 * User Schema Migration V1 - Browser Version
 * 
 * Safely adds all new schema v1 fields to existing user documents.
 * Uses client SDK - must be run from browser console while logged in as admin.
 * Can be run multiple times - only updates missing fields.
 * 
 * USAGE:
 * 1. Login to admin panel at http://localhost:3001/admin
...
(async function migrateUsersSchemaV1() {
  ...
  try {
    ...
    const { getFirestore, collection, getDocs, doc, updateDoc, Timestamp } = await import('firebase/firestore');
    const { db } = await import('../services/firebase.ts');
    
    if (!db) {
      throw new Error('Firebase not initialized. Make sure you are logged in to the admin panel.');
    }
    ...
    function getMissingFields(userData, userId) {
      ...
      // Timestamps
      if (!userData.createdAt) updates.createdAt = userData.joinDate || now;
      if (!userData.updatedAt) updates.updatedAt = now;
      if (!userData.lastLoginAt) updates.lastLoginAt = userData.createdAt || now;
      ...
      if (!userData.passwordUpdatedAt) updates.passwordUpdatedAt = userData.createdAt || now;
      ...
      // Compliance
      if (!userData.termsAcceptedAt) updates.termsAcceptedAt = userData.createdAt || now;
      if (!userData.privacyAcceptedAt) updates.privacyAcceptedAt = userData.createdAt || now;
    ...
    for (let i = 0; i < users.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (userDoc) => {
        ...
        try {
          ...
          Object.keys(missingFields).slice(0, 10).forEach(key => {
          ...
          if (Object.keys(missingFields).length > 10) {
            console.log(`     ... and ${Object.keys(missingFields).length - 10} more fields`);
          }

          // Update Firestore
          await updateDoc(doc(db, 'users', userId), missingFields);
          updatedCount++;
    ...
    if (errorCount > 0) {
      ...
      console.log('💡 Reload the page to see updated users.');
    }
