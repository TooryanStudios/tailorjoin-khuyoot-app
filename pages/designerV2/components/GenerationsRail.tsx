  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-black text-slate-900 dark:text-white">إبداعاتي ({generations.length})</div>
      </div>
        ...
        {generations.slice(0, 15).map((g) => {
          ...
          return (
            <button
              ...
              className="group relative w-[84px] aspect-[3/4] shrink-0 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 hover:opacity-95"
              title="خيارات"
            >
              <img
                  ...
                  <div className="flex flex-col items-center justify-center gap-1.5">
                    {onSetBefore ? (
                      <button
                        ...
                        className="w-7 h-7 rounded-md bg-white/90 text-slate-900 flex items-center justify-center hover:bg-white transition-colors"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          onSetBefore(g.url);
